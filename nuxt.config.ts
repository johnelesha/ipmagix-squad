...
const enFiles = existsSync(enDir) ? readdirSync(enDir).filter(file => file.endsWith('.json')).map(file => `en/${file}`) : [];
const arFiles = existsSync(arDir) ? readdirSync(arDir).filter(file => file.endsWith('.json')).map(file => `ar/${file}`) : [];
